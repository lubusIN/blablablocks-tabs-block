...
function Placeholder({ clientId, attributes }) {
    ...
    const handleStartBlank = useCallback(() => {
        try {
            const defaultTemplate = [
                ['core/paragraph', { placeholder: 'Type / to add a block in the tab' }],
            ];
            const blocks = createBlocksFromInnerBlocksTemplate(defaultTemplate);
            replaceInnerBlocks(clientId, blocks, true);
            setStep('blank');
        } catch (err) {
            setError(__('Failed to create blank tab. Please try again.', 'blablablocks-tabs-block'));
        }
    ...
    const applyPattern = useCallback((pattern) => {
        if (!pattern || !pattern.content) {
            setError(__('Invalid pattern selected. Please choose another pattern.', 'blablablocks-tabs-block'));
            return;
        }
        ...
        try {
            ...
            setError(__('Failed to apply pattern. Please try another one.', 'blablablocks-tabs-block'));
        }
    }, [clientId, replaceInnerBlocks]);
        ...
        <div {...blockProps}>
            ...
            {!step && (
                <PlaceholderComponent
                    ...
                    label={__(`Tab ${attributes.tabname ?? ''}`, 'blablablocks-tabs-block')}
                >
                    <Button
                        variant="primary"
