...
export default function Edit({ clientId, attributes, setAttributes }) {
    ...
    const validateSvg = (svgCode) => {
        if (!svgCode.trim()) {
            setValidationError(__('Please paste your SVG code.', 'blablablocks-tabs-block'));
            return false;
        }
        ...
        if (!svgRegex.test(svgCode)) {
            setValidationError(__('Invalid SVG code. Please provide a valid SVG.', 'blablablocks-tabs-block'));
            return false;
        }
     ...
     */
    const { hasChildBlocks, tabsClientId, parentAttributes, siblingTabs } = useSelect(
        (select) => {
            const { getBlockOrder, getBlockRootClientId, getBlockParentsByBlockName, getBlock } = select('core/block-editor');
     ...
     *
     * @param {string} clientId - The client ID of the tab to set as default.
     * @param {boolean} value - The value to set for the `isDefault` attribute.
     */
    const handleSetDefault = (clientId, value) => {
    ...
    return (
        <div {...blockProps}>
                ...
                <div className='blablablock-tab-btn'>
                    ...
                    {attributes.tabIcon && (
                        <span
                            ...
                            dangerouslySetInnerHTML={{ __html: attributes.tabIcon }}
                        />
                    )}
                    {/* Render the tab name */}
                    <RichText
            ...
            <InspectorControls>
                ...
                >
                    <ToggleControl
                        label={__('Set as default tab', 'blablablocks-tabs-block')}
                        checked={attributes.isDefault}
                        onChange={(value) => handleSetDefault(clientId, value)}
                    />
                </PanelBody>
            </InspectorControls>
            ...
            {isOpen && (
                <Modal className='bbb-custom-icon-modal' title={attributes.tabIcon ? __('Edit Custom Icon', 'blablablocks-tabs-block') : __('Add Custom Icon', 'blablablocks-tabs-block')} isFullScreen onRequestClose={closeModal}>
                    <Flex align='stretch' gap={5} style={{ height: '100%' }}>
                        ...
                        <FlexBlock>
                            <VStack spacing={5} justify='space-between' style={{ height: '100%' }}>
                                <VStack spacing={5} >
                                    <Card style={{ height: '200px' }} isRounded={false}>
                                        <div className='bbb-icon-preview' dangerouslySetInnerHTML={{ __html: svgCode || attributes.tabIcon }} />
                                    </Card>
                                    {validationError && (
                                ...
                                <HStack justify='flex-end'>
                                    <Button
                                        ...
                                        disabled={!isSvgValid}
                                    >
                                        {__('Insert custom icon', 'blablablocks-tabs-block')}
                                    </Button>
                                </HStack>
                            </VStack>
                        </FlexBlock>
