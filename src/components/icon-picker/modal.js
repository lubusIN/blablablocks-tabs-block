...
function IconPickerModal({
	...
	const validateSvg = (svg) => {
		...
		if (!matches || matches.length === 0) {
			setValidationError(__('Invalid SVG code. Please provide valid SVG(s).', 'blablablocks-tabs-block'));
			return false;
		}
		...
		if (joinedSVGs !== trimmed) {
			setValidationError(__('Only SVG code is allowed. Remove any extra text outside <svg> tags.', 'blablablocks-tabs-block'));
			return false;
		}
	...
	return (
		...
		>
			<Grid align="stretch" templateColumns={'auto 300px'} gap={5} style={{ height: '100%' }}>
				<TextareaControl
						...
						>
							{isSvgValid ? (
								...
								<div className="bbb-icon-preview">
									{__('SVG Preview', 'blablablocks-tabs-block')}
								</div>
							)}
						</Card>
						{validationError && (
