...
const PatternSidebar = ({ selectedCategory, setSelectedCategory, searchTerm, setSearchTerm }) => {
    ...
    const { patternCategories, blockPatterns, error } = useSelect((select) => {
        try {
    ...
    if (error) {
        return (
            <div className="bbb-tabs-patterns-sidebar--error">
                {__('Failed to load block patterns.', 'blablablocks-tabs-block')}
            </div>
        );
    }
    ...
    const filteredCategories = useMemo(() => {
        return patternCategories.filter((category) =>
            blockPatterns.some(
                (pattern) =>
                    Array.isArray(pattern.categories) && pattern.categories.includes(category.name)
            )
        );
