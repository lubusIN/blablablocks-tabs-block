...
const PatternList = ({ clientId, selectedCategory, searchTerm, onSelect }) => {
    ...
    const filteredPatterns = useMemo(() => {
        ...
        return patterns.filter((pattern) => {
            ...
            const matchesSearch = pattern.title &&
    ...
    const handlePatternSelect = (pattern) => {
        if (pattern && typeof onSelect === 'function') {
            try {
                ...
                setError(__('Failed to select pattern. Please try again.', 'blablablocks-tabs-block'));
            }
        }
    };
                ...
                <>
                    {!filteredPatterns?.length ? (
                        <p className="tabs-patterns-no-results">
                            {__('No patterns found.', 'blablablocks-tabs-block')}
                        </p>
                    ) : (
                        <>
                            <Grid gap={8} columns={[1, 2, 3]} align="start" className="tabs-patterns-grid-content">
                                {paginatedPatterns.map((pattern) => (
                                    <Button
                                        key={pattern.name || `pattern-${pattern.id || Math.random()}`}
                                        ...
                                        >
                                            {pattern.content ? (
                                                ...
                                                <div className="tabs-patterns-preview-error">
                                                    {__('Preview not available', 'blablablocks-tabs-block')}
                                                </div>
                                            )}
                                            <Text align="left" size={12}>
                                                {pattern.title || __('Untitled pattern', 'blablablocks-tabs-block')}
                                            </Text>
                                        </VStack>
                                    </Button>
                                ))}
